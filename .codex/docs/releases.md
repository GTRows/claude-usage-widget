...
- **Versioning.** Semver only. Tag format `v<version>`. Pre-release suffixes (`-rc.1`, `-beta.2`, ...) must match `IDENTITY.yaml#release.prerelease_tags` â€” workflow detects and flags the release as pre-release automatically.
...
3. Rotates `CHANGELOG.md`: renames `## [Unreleased]` â†’ `## [version] - <date>`, inserts a fresh `## [Unreleased]`.
4. Syncs `package.json` / `pyproject.toml` / `Cargo.toml` version if they exist.
5. Commits and tags locally. Does **not** push â€” you review, then `git push && git push --tags`.
