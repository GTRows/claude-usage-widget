...
The orchestrator pipes the worker's full output through `python .codex/scripts/orchestrate_protocol.py parse` to extract the JSON. Everything before the tail is context-only â€” it never enters the orchestrator's window.
...
| `files`    | optional | executor                 | int â€” files changed in the plan's commit range       |
...
Note: a verifier `verdict: fail` still emits `status: ok` because the verifier itself succeeded â€” it just observed broken work. `status: blocked` is reserved for "I literally cannot proceed without help" (e.g. no test command exists, plan is unreadable).
...
This is the natural release breakpoint â€” you decide whether what was just built is ship-worthy. `release` invokes `/gtr:release <v>`; `skip-release` continues without releasing; `stop` exits cleanly. `forever` mode and the `--auto-release` flag skip this prompt and pick `release`.
...
- Token budget exhausted â†’ Codex crash. Resume by running `/gtr:orchestrate resume` in a fresh session.
- User Ctrl+C.
...
1. `.planning/STATE.md` â€” the GSD cursor (current phase, last action).
2. Git log since the last `chore(release):` commit â€” matches commits to plan task IDs.
3. `.planning/phases/*/SUMMARY.md` â€” closed plans.
...
- **Sequential, not parallel.** `Task` calls block until the worker exits. This isn't a performance optimisation â€” it's a context-isolation trick. If you want parallelism for independent work, see `oh-my-claudecode:ultrawork`, but be aware GSD's roadmap is usually sequentially dependent.
...
- **A worker's "I'm done" report is intent, not truth.** That's why `verifier` exists â€” independent confirmation. Don't disable it (`--no-verifier`) unless you have a strong external check.
...
- **Subagent token costs.** Each spawn pays the system-prompt cost for the agent. Cheap relative to the alternative (long single-window context with auto-compact), but it is a cost â€” not free.
...
```bash
python .codex/scripts/orchestrate_protocol.py emit --status ok --field path=foo --field summary="hi" \
  | python .codex/scripts/orchestrate_protocol.py parse
```

Should print the JSON back. If not, something is wrong with the script â€” file an issue.
