...
Files under `.codex/commands/*.md` become `/<name>` slash commands. Files under `.codex/commands/<ns>/*.md` become `/<ns>:<name>` slash commands. The frontmatter `description` shows in `/help`. The body is the prompt Claude runs.

## Included in this template

| Command | Purpose |
|---------|---------|
| `/gtr:menu` | Interactive entry point â€” pick what to do, Claude routes to the right command. |
| `/gtr:setup` | First-time project wizard. Detects stack, fills AGENTS.md, installs plugins, hands off planning to GSD, writes setup marker. |
...
Name project commands to avoid collisions with Anthropic plugins (e.g. `/review` is taken by the pr-review-toolkit plugin â€” use template-prefixed names).
...
| `codex-md-management@claude-plugins-official` | `/revise-Codex-md` + claude-md-improver skill |
...
| `oh-my-claudecode@omc` | Autopilot, ralph, ultrawork, deep-dive, plan, team â€” and **GSD planning** which the template hands off to. |
...
Plugin-based skills show up as `plugin-name:skill-name` or directly as `skill-name` in the available-skills list. Do not copy plugin source into `.codex/skills/` â€” keep plugins global and only bundle project-specific skills into `.codex/skills/`.

## Task trackers (compatible, not bundled)

Vibe Kanban and similar external task boards are compatible with this template's `.planning/` flow â€” they are not mutually exclusive. Treat external boards as the source of truth for cross-team work; keep `.planning/` for the handful of items the AI assistant is actively driving. Do not mirror one into the other.
