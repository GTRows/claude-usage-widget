...
3. Working tree is clean (`git status --porcelain` empty). If not, abort: "Commit or stash first â€” orchestration writes commits."
...
For `resume`: skip preflight check 3 (clean tree) â€” resume can pick up after a crash that left files mid-write. But warn the user and ask for confirmation.
...
have no Write or Edit â€” do not patch anything.
```
...
```bash
echo "{worker-output}" | python .codex/scripts/orchestrate_protocol.py parse
```
...
- A `Task` call itself errors (not the worker â€” the tool). Print the error, stop.
...
- Workers know the protocol from their agent definitions in `.codex/agents/`. You don't need to repeat the protocol contract in every spawn prompt â€” keep prompts short.
