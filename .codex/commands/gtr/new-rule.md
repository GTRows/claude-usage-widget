---
description: "[TEMPLATE] Scaffold a path-scoped AGENTS.md rule under .codex/rules/."
---
...
Path-scoped rules let you keep `AGENTS.md` short and load extra rules only when relevant files are being edited. They live under `.codex/rules/<slug>.md` and use frontmatter to declare their scope.
...
2. Refuse if `.codex/rules/<slug>.md` already exists.
3. Ask the user for the path glob(s) the rule applies to (e.g. `tests/**/*.py`, `src/api/**`).
4. Create `.codex/rules/<slug>.md` using the template below.
...
- <what NOT to do â€” Codex's likely default that this rule overrides>
...
- Rules under `.codex/rules/` are tracked in git. They are not personal â€” they are project conventions.

## When NOT to make a rule

- The guidance is universal across the whole project â†’ put it in `AGENTS.md` instead.
- The guidance is your personal preference â†’ put it in `~/.codex/AGENTS.md`.
- The guidance is a one-time decision â†’ use `/gtr:new-adr` instead (decisions live in ADRs, ongoing rules live here).
