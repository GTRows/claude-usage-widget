...
| **truly nothing** | no migration tool detected | â€” | stop and ask the user which tool to use; do not invent one. |
...
- Set explicit constraints (`NOT NULL`, `DEFAULT`, `CHECK`, `UNIQUE`) â€” do not rely on the dialect's defaults.
