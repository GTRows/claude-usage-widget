...
1. `.codex/.setup-complete` must exist â€” refuse if the project has not been set up.
2. Working tree must be clean (`git status --porcelain` is empty). Refuse with: `Commit or stash first â€” /gtr:update needs a clean tree to write a single update commit.`
...
## Step 1 â€” Find the upstream version

Tags are the source of truth â€” `gh release list` only sees published GitHub Releases and lags behind tags pushed without a release. Always query tags first:
...
- Equal â†’ tell the user `Already on latest (vX.Y.Z).` and stop.
- Local newer â†’ tell the user `Local version (X) is ahead of upstream (Y). Nothing to pull.` and stop.
- Upstream newer â†’ continue.

## Step 2 â€” Check the manifest
...
```json
{
  ...
  "files": {
    ...
    ".codex/commands/setup.md": "<sha256-at-install>",
...
1. Tell the user explicitly:
   > No `.codex/.template-manifest.json` found. I will seed one from the current local files. Any file you have already modified will be treated as "untouched", so on the next conflict step I will not be able to detect those edits â€” you will see them as auto-update candidates instead. This is a one-time bootstrap.
...
## Step 2.5 â€” Run schema migrations (BEFORE fetching upstream)
...
diff cannot â€” directory renames, dropped files, identity rename, etc.

1. **Preview** what will run:
   ```bash
   python .codex/scripts/migrations.py --target <upstream-version> --check
   ```
...
4. On `yes`:
   ```bash
   python .codex/scripts/migrations.py --target <upstream-version>
   ```
...
Read the upstream `.codex/.template-manifest.json` if present, otherwise compute hashes ad-hoc for the relevant template-owned paths.

## Step 4 â€” File-by-file decision

For each path that exists in upstream **and** is template-owned (see exclusion list below):

| State | Action |
|-------|--------|
| Local sha == manifest sha (user has not modified) AND upstream sha differs | **Auto-update** â€” write upstream content. |
| Local sha == manifest sha AND upstream sha matches | **Skip** â€” no change. |
| Local sha != manifest sha (user modified) AND upstream sha == manifest sha | **Skip** â€” only the user changed. |
| Local sha != manifest sha AND upstream sha differs | **Conflict** â€” show 3-way summary (manifest â†’ local diff, manifest â†’ upstream diff). Ask: `keep / replace / merge-by-hand / skip`. |
| Path missing locally, present upstream | **Add** â€” write file. |
| Path present locally, missing upstream | Ask `Upstream removed this file â€” delete locally?` â€” default `keep`. |

**Excluded paths** (never touched by `/gtr:update`):

```
CHANGELOG.md
IDENTITY.yaml         (identity is per-project â€” was PROJECT.yaml in v0.1.x)
PROJECT.yaml          (legacy alias; only present pre-v0.2.0)
README.md             (per-project doc)
.codex/.setup-complete
.codex/settings.local.json
.gitignore            (merge by appending only â€” never overwrite)
.github/workflows/*.yml         (activated workflows are per-project â€” only touch *.yml.template)
.env*
LICENSE
```
...
## Step 5 â€” Apply, refresh manifest, commit
...
```
Template updated: v<old> â†’ v<new>
...
- If the upstream removes a hook the user still has registered in `settings.json`, do NOT silently unregister â€” ask.
