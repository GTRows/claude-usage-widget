---
description: "[TEMPLATE] Interactive menu â€” pick what you want to do, Claude routes to the right command (template or GSD)."
---
...
**Output budget.** Follow `.codex/docs/output-style.md`. The menu itself is the response â€” no preamble, no closing note. Just the menu and the prompt for a number.
...
2. **Adapt the menu:**
   - If `.codex/.setup-complete` is missing â†’ option 1 stays primary, gray out everything else with `(run /gtr:setup first)` annotation.
   - If `.planning/` is missing â†’ planning options 7-12 invite "start a new plan" rather than "continue".
   - If no identity file exists â†’ hide release options.
...
What do you want to do?
  ...
  Reference
   ...
   15. Refresh AGENTS.md                         /revise-Codex-md (plugin)
```
...
|------|--------------------|-------------------|
| 3 (set language) | `Which language? (English / TÃ¼rkÃ§e / Deutsch / ... â€” leave empty to be asked)` | `/gtr:set-language [lang]` |
...
First time? Recommended path:
  1 â†’ /gtr:setup           (template scaffolding)
  7 â†’ /gsd:new-project     (project vision and roadmap)
 10 â†’ /gsd:plan-phase 1    (concrete plan)
 11 â†’ /gsd:execute-plan    (build it)
