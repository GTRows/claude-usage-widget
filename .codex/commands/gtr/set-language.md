...
`$ARGUMENTS` may already contain the language. Accept English names (`Turkish`, `English`, `German`), native names (`TÃ¼rkÃ§e`, `Deutsch`, `FranÃ§ais`), or ISO 639-1 codes (`tr`, `en`, `de`). Normalise to the English name (e.g. `tr` -> `Turkish`, `TÃ¼rkÃ§e` -> `Turkish`).

If `$ARGUMENTS` is empty, ask exactly:

> Which language should I speak with you in this project? (English / TÃ¼rkÃ§e / Deutsch / FranÃ§ais / EspaÃ±ol / ... â€” pick one)
...
For Turkish: `KonuÅŸma dili TÃ¼rkÃ§e olarak ayarlandÄ±. Bundan sonraki /gtr:* ve /gsd:* Ã§Ä±ktÄ±larÄ± bu dilde olacak.`
...
- Does **not** restart Codex or reload its session â€” the new language takes effect on the next `/gtr:*` or `/gsd:*` invocation that reads `## Communication`.
