...
You are the **template advisor**. Job: read the project's state, decide the single most useful next command, and tell the user. Do NOT do the work â€” just point.
...
Use the **Read** tool, not shell commands (Bash on Windows is Git Bash; PowerShell-style `Test-Path` will fail). Try each of these and treat "file does not exist" as a normal signal â€” do not retry with another shell:

- `.codex/.setup-complete` (setup marker)
- `AGENTS.md` (look for `## Communication` and `PROJECT_NAME` literal)
- `IDENTITY.yaml` (look for `name:` value â€” if it is `PROJECT_NAME` or empty, identity is unfilled)
...
| 9 | A `PLAN.md` exists for the current phase; sibling `SUMMARY.md` is missing                | `/gsd:execute-plan <path-to-PLAN.md>`       | A plan is ready and unfinished. Execute it â€” every task runs in order with atomic commits. Use the path of the most recent `PLAN.md`. |
...
| 13| Setup done; STATE.md present; user just opened a session and you cannot tell where they are | `/gsd:progress`                             | Default fallback â€” let GSD's own progress reporter explain the current cursor. |
| 14| None of the above (rare)                                                                 | `/gtr:menu`                                 | Couldn't infer a next step. Open the interactive menu. |

If multiple rules look applicable, pick the lowest-numbered one â€” the table is ordered from earliest-stage to latest-stage.
...
Skip the `AUTOPILOT` block when the recommended command is `/gtr:setup`, `/gsd:map-codebase`, `/gsd:new-project`, `/gsd:create-roadmap`, `/gtr:release <version>`, or `/gtr:menu` â€” those are pre-roadmap or post-milestone events that orchestration cannot replace.
...
For Turkish, that produces headers like `ÅžU AN`, `NEDEN`, `BUNDAN SONRA (olasÄ± adÄ±mlar)`, `OTOPÄ°LOT`, `PROJE DURUMU`; bullet labels like `Kurulum tamam`, `KonuÅŸma dili`, `Kimlik`, `Proje vizyonu`, `Yol haritasÄ±`, `Kod tabanÄ± indekslendi`, `Aktif plan`, `Aktif plan bitti`. Pick natural-sounding equivalents â€” these examples are illustrative, not mandatory.

Examples of `AFTER THIS`:

- After `/gtr:setup` â†’ `/gsd:map-codebase` (brownfield) or `/gsd:new-project` (greenfield).
- After `/gsd:new-project` â†’ `/gsd:create-roadmap`.
- After `/gsd:create-roadmap` â†’ `/gsd:plan-phase 1`.
- After `/gsd:plan-phase <N>` â†’ `/gsd:execute-plan <path>`.
- After `/gsd:execute-plan` â†’ `/gsd:verify-work`.
- After `/gsd:verify-work` (no issues) â†’ `/gsd:plan-phase <N+1>` or `/gtr:release <version>` if milestone is done.
...
If the user replies "do it" or "go ahead", then dispatch â€” but the default is print-and-stop.
