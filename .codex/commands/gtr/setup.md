---
description: "[TEMPLATE] First-time project setup wizard. Detects stack, fills AGENTS.md, installs plugins, hands off planning to GSD, writes setup marker."
---

You are the **template setup wizard**. Run this once per project. It is idempotent â€” safe to re-run to refresh detected values.

**Output budget.** Follow `.codex/docs/output-style.md`. Each step asks one question or makes one announcement â€” no recap, no narration. The final summary stays under ~10 lines.

`$ARGUMENTS` may include `--extras`. When present, **skip steps 1â€“12 entirely** and run only step 13 (Optional scaffolding) plus the closing manifest refresh and summary. Step 1 (conversation language) still applies â€” read the existing `## Communication` section in `AGENTS.md` and continue in that language. Only re-ask the language question if no `## Communication` section exists yet.
...
> What language should I speak with you in this project? (English / TÃ¼rkÃ§e / Deutsch / FranÃ§ais / EspaÃ±ol / ... â€” pick one)

Default suggestion: detect from the language the user wrote `/gtr:setup` in and propose it as the default. If the user typed in Turkish, default to TÃ¼rkÃ§e; if in English, default to English; etc.
...
If the wizard is being re-run and `## Communication` already exists, skip this step silently and continue in the already-configured language â€” but still confirm with one short sentence: "Continuing in <Language>." Then proceed.

---

## 2. Preflight

Check if `.codex/.setup-complete` exists. Use the **Read** tool, not shell commands. (Bash on Windows is Git Bash; PowerShell-style `Test-Path` will fail.) If the Read returns "file does not exist", treat that as Missing â€” do not retry with another shell.

- **Exists** â†’ tell the user setup was already done (show `date` from the marker). Ask if they want to re-run. If no, stop.
- **Missing** â†’ continue.
...
- `package.json` (Node/JS/TS â€” inspect `dependencies`, `scripts`, `engines`)
...
| `release.platforms` | Infer from stack (Electron â†’ windows+macos; CLI binary â†’ windows-x64+linux-x64+macos-arm64; web-only â†’ empty). |
...
- `PROJECT_NAME` â†’ `identity.name`
- `Description` â†’ `identity.description`
- `Tech Stack` â†’ from detection
- `Platform` â†’ from detection
...
**Sections to leave placeholder** when the code doesn't tell you â€” add a line to `.codex/setup-followups.md` instead of fabricating:
...
- **no** â†’ no further action.
- **later** â†’ add a `setup-i18n` follow-up line to `.codex/setup-followups.md`: "i18n strategy decided".
- **yes** â†’ ask one follow-up: `Which UI languages will you support? (comma-separated, e.g. en, tr, de)`. Then append to `AGENTS.md` under `## Localization`:
  ```
  - Supported UI languages: <list>.
  - Default / fallback: <first in list>.
  - Translation source format: TBD (gettext, JSON, ICU, ...) â€” set when picking the i18n library.
...
| `codex-md-management` | `claude-plugins-official` | `/revise-Codex-md` + improver skill |
...
**Step 9a â€” register custom marketplaces first.** Run `claude plugin marketplace list`; if `omc` is missing, add it:

```bash
claude plugin marketplace add https://github.com/Yeachan-Heo/oh-my-claudecode
```

The other two marketplaces (`claude-plugins-official`, `claude-code-plugins`) ship with Codex; you do not need to add them.

**Step 9b â€” install plugins.** Run `claude plugin list` first. Skip anything already installed. Install the rest:

```bash
claude plugin install <name>@<marketplace>
```

Users running Codex with `--dangerously-skip-permissions` get silent installs; otherwise they get one prompt per plugin.

**Step 9c â€” pin the installed set.** After installs land, write the pin file so `/gtr:doctor` can flag drift later:

```bash
python .codex/scripts/plugins.py --pin
```

This records `name@marketplace` for every currently installed plugin into `.codex/plugin-pin.json` (committed). The next `/gtr:doctor` run uses it to detect uninstalled or missing plugins.
...
> - **yes**   â†’ I will run `/gsd:new-project` next (and `/gsd:map-codebase` first if this is an existing codebase). The brief is auto-filled from `IDENTITY.yaml` / `AGENTS.md` / `README.md`; you only confirm or tweak.
> - **no**    â†’ skip planning. Template still works for hooks, releases, onboarding, doctor.
> - **later** â†’ captured in `.codex/setup-followups.md` (gitignored). `/gtr:doctor` surfaces it.
...
Do **NOT** create `TODO.md` or `DEFERRED.md` â€” both are removed in v0.2.0. Planning lives under `.planning/` (managed by GSD) or not at all.
...
- **no** â†’ skip.
- **later** â†’ add `setup-ci` follow-up to `.codex/setup-followups.md`: "CI scaffolding decision revisited".
- **yes** â†’
  1. If `.github/workflows/ci.yml` is missing and `ci.yml.template` exists, copy template â†’ `ci.yml`. Do NOT delete the `.template`.
...
- **no** â†’ skip entirely. Do not create `CHANGELOG.md`, `RELEASE.md`, or the workflow.
- **later** â†’ add `setup-release` follow-up to `.codex/setup-followups.md`: "release scaffolding decision revisited".
- **yes** â†’
  ...
  3. If `.github/workflows/release.yml` is missing and `release.yml.template` exists, copy template â†’ `release.yml`. Do NOT delete the `.template`.
...
>
>   1. `LICENSE` (MIT default â€” ask the year and copyright holder)
...
>   6. `.pre-commit-config.yaml` (pre-commit framework â€” runs lint/format on staged files)
>   7. `.env.example` (env-var template; only meaningful if the project uses dotenv)

For each picked option:

- `LICENSE` â†’ write the MIT text with `<year>` / `<owner>` filled in. Update `IDENTITY.yaml#identity.license` to `MIT`.
- `SECURITY.md` â†’ minimal disclosure policy (email contact, expected response time, supported versions table â€” fill from `IDENTITY.yaml`).
- `CONTRIBUTING.md` â†’ derive from this AGENTS.md's `Git and Commits`, `Code Standards`, `File Organization` sections.
- `.github/CODEOWNERS` â†’ ask for the user's GitHub handle, default `* @<handle>`.
- `.github/dependabot.yml` â†’ infer ecosystem from detection in step 3 (npm, pip, cargo, gomod, ...).
- `.pre-commit-config.yaml` â†’ infer hooks from stack (ruff for Python, prettier for JS, etc.); leave a comment block of suggested hooks the user can opt into.
- `.env.example` â†’ only if the project already imports `dotenv` / uses `os.environ.get` patterns. Scan top-level source for variable names and stub them with empty values.

For files the user did not pick, do nothing â€” do not add follow-ups for them.
...
Ensure `.codex/.setup-complete` is listed in `.gitignore` â€” the marker is per-clone, not per-repo.

Then write the template manifest so `/gtr:update` can later detect which files the user has modified:

```bash
python .codex/scripts/manifest.py --write
```
...
- Next step â€” usually `/gsd:new-project` (if planning was opted in) or `/gtr:menu` for the option list
