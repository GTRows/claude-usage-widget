---
description: "[TEMPLATE] Scaffold a project-specific Codex skill under .codex/skills/."
---
...
Project-specific skills live under `.codex/skills/<slug>/`. They differ from plugin-provided skills:
...
- **Project skills** stay inside `.codex/skills/`, tracked in git. Reach for them when the capability is specific to *this* project's domain (e.g. domain glossary, internal API patterns, deployment runbook).

If the user asks for something that is general-purpose, suggest `/skill-creator:skill-creator` (plugin) instead â€” do not bloat `.codex/skills/`.

## Steps

1. Slugify the title from `$ARGUMENTS` (lowercase, hyphenated, ASCII).
2. Refuse if `.codex/skills/<slug>/` already exists.
3. Create the directory and write `.codex/skills/<slug>/SKILL.md` using the template below.
...
- Trigger 1 (concrete signal â€” file pattern, keyword, error message).
- Trigger 2.
...
would otherwise have to re-explain every session. Be concrete and short â€”
...
- Keep `SKILL.md` under 200 lines. Skills are loaded into context â€” long ones are expensive.
