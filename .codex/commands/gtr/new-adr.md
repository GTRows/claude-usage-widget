...
1. Ensure `docs/adr/` exists. If not, create it and seed `docs/adr/0001-record-architecture-decisions.md` (a meta-ADR explaining the team adopts ADRs) â€” only on the very first call.
...
- <Concrete benefits â€” capabilities unlocked, simplifications, etc.>

### Negative

- <Concrete costs â€” complexity added, future migrations needed, lock-in.>
...
`Proposed` â†’ `Accepted` â†’ (later) `Deprecated` or `Superseded by <NNNN>`. Never delete an old ADR; supersede it. Old ADRs are history, not garbage.
...
- Date stamps use `YYYY-MM-DD`, status changes append a line at the top of the file with the new status and date â€” keep history.
