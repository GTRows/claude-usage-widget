...
def plan(root: Path) -> list[Action]:
    ...
    if cmds.is_dir() and not snapshot.exists():
        ...
        # at the OLD location â€” otherwise the project is already migrated.
    ...
    # 6. Slash-command renames in text files (preview only â€” apply checks per file).
    ...
    # 7. /task references that cannot be auto-migrated â€” surface them.
        ...
        actions.append(
            Action(
                ...
                note=(
                    ...
                    + ", ".join(task_refs)
                    + " â€” these cannot be auto-migrated to GSD. Update manually after running /gsd:new-project."
                ),
            )
        )
