...
def test_plan_idempotent_on_clean_v0_2_layout(tmp_path):
    # Clean v0.2 layout â€” no v0.1 commands present, gtr/ is the only dir,
...
LEGACY_COMM = (
    "## Communication\n\n"
    "- Speak with the user in Turkish (TÃ¼rkÃ§e). All conversational responses,\n"
...
def _write_agents_md(root: Path, body: str) -> Path:
    f = root / "AGENTS.md"
...
def test_v064_plan_no_op_when_legacy_block_absent(tmp_path):
    _write_agents_md(tmp_path, "# AGENTS.md\n\nNo communication section.\n")
...
def test_v064_plan_detects_legacy_block(tmp_path):
    ...
    assert actions[0].src == "AGENTS.md"


def test_v064_apply_replaces_legacy_block(tmp_path):
    f = _write_agents_md(tmp_path, "# AGENTS.md\n\n" + LEGACY_COMM + "\n## Next section\n\nbody.\n")
    actions = v064.plan(tmp_path)
