...
- Plugin commands: `/commit`, `/commit-push-pr`, `/review-pr`, `/revise-Codex-md`.
...
**Name:** Claude/Codex usage widget (GTRows fork)
**Description:** Desktop widget for Claude.ai and Codex/OpenAI usage monitoring. Lightweight always-on overlay that polls the active account, plots windowed throughput and session burn-rate, and surfaces quota and usage notifications.
...
- User settings, account metadata, window geometry, and cached samples live in electron-store under the OS user data directory (`%APPDATA%\claude-usage-widget` on Windows, `~/Library/Application Support/claude-usage-widget` on macOS, `~/.config/claude-usage-widget` on Linux).
- No telemetry. Network access is limited to the official provider endpoints needed for the active Claude or Codex/OpenAI account.
...
If you need to extend the protected set, update `PROTECTED_EXACT` in `.codex/hooks/pre_guard_release_files.py`.
...
- Commits are authored by the user via local git config. Do NOT add `Co-Authored-By: Codex` trailers.
