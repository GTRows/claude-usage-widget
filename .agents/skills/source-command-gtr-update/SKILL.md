...
Use this skill when the user asks to run the migrated source command `gtr-update`.

## Command Template

Pull updates from the upstream template repo (`https://github.com/GTRows/Codex-template`) into the current project, merge them into existing files when safe, and ask before overwriting anything the user has modified.
...
```bash
git ls-remote --tags https://github.com/GTRows/Codex-template.git \
...
```bash
gh release list --repo GTRows/Codex-template --limit 1 --json tagName --jq '.[0].tagName'
```
...
git clone --depth 1 --branch v<upstream-version> \
  https://github.com/GTRows/Codex-template.git "$(mktemp -d)/ccp-update"
```
