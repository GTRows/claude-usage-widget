...
Use this skill when the user asks to run the migrated source command `gtr-onboard`.
...
```
Onboarding from https://github.com/GTRows/Codex-template
...
```bash
TEMPLATE_TAG=$(gh release list --repo GTRows/Codex-template --limit 1 --json tagName --jq '.[0].tagName' 2>/dev/null \
  || git ls-remote --tags https://github.com/GTRows/Codex-template.git | awk -F/ '{print $NF}' | grep -E '^v[0-9]+\.[0-9]+\.[0-9]+$' | sort -V | tail -1)
TEMPLATE_DIR=$(mktemp -d)/ccp-onboard
git clone --depth 1 --branch "${TEMPLATE_TAG}" https://github.com/GTRows/Codex-template.git "${TEMPLATE_DIR}"
```
...
Copy every `.Codex/hooks/*.py` and `.Codex/hooks/optional/*.py` if missing locally. If a hook with the same name exists locally, show the diff and ask.

### 2b. Slash commands (always safe)

Copy every `.Codex/commands/*.md`. If a command with the same name exists locally, show the diff and ask.

### 2c. Scripts and tests

Copy `.Codex/scripts/manifest.py` and `.Codex/hooks/tests/`.
...
git commit -m "chore: adopt Codex-template (${TEMPLATE_TAG})"
```
