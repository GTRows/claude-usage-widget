...
3. Capture the worker's full output text. Pass it to `python .Codex/scripts/orchestrate_protocol.py parse` (stdin) to extract the tail-block JSON. If parsing fails, treat as `{"status":"fail","reason":"missing tail-block"}` and retry once with stricter wording.
...
- Token exhaustion → Codex session crash. Resume by re-running `/gtr:orchestrate resume`.
